...
(async function () {
  ...
  assert(
    initialX + moveUpBy >= 0 && initialX + moveUpBy <= 200,
    "Avatar went out of bounds"
  );
  ...
  response = await sendAndWait({
    action: "moveRight",
    id: avatarId,
    distance: moveRightBy,
  });
  assert.strictEqual(response, "success", "Failed to move avatar right");
  assert(
    initialY + moveRightBy >= 0 && initialY + moveRightBy <= 200,
    "Avatar went out of bounds"
  );
  ...
  console.log(
    `Avatar ${avatarId} is at position (${position.x}, ${position.y})`
  );
  ...
  assert.strictEqual(
    response,
    "success",
    "Failed to move avatar to the bottom"
  );
  ...
  assert(
    newPos.x === 200 && newPos.y === 200,
    "Avatar3 position is incorrect after moving"
  );

  // 11. Move non-existent avatar
  response = await sendAndWait({
    action: "moveUp",
    id: "nonExistentAvatar",
    distance: 10,
  });
  assert.strictEqual(
    response,
    "error: Avatar not found",
    "Allowed moving a non-existent avatar"
  );
  ...
  assert.strictEqual(
    response,
    "success",
    "Failed to move avatar with large negative value"
  );
  response = await sendAndWait({ action: "position", id: "avatar3" });
  assert(
    JSON.parse(response).x === 0,
    "Avatar3 moved out of bounds with large negative value"
  );
  ...
  assert.strictEqual(
    response,
    "error: Out of bounds",
    "Allowed avatar spawn with negative coordinates"
  );
